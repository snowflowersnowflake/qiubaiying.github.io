...
* 在校参与智慧建筑相关科研课题，熟悉[定制化AI平台easydl](https://ai.baidu.com/easydl/mobile),同时了解numpy、pandas、plt、seaborn、sklearn、keras（基于TensorFlow封装）等常用python第三方库的基本使用方法（博客中包含学习笔记），部分实验存放在github仓库，对机器学习算法，部分深度学习模型（主要图像处理相关）有一定的理论认识，但Ai算法相关实战经历远远不够，渴望学习求索。
...
* 打字很快，但只使用双拼输入方式，很难全拼输入（敲重点）
* 乐于助人，例如为班级设计贴纸（存在电脑中自恋为艺术品），上大学那年因为路上看到的不知名少女疑似拐卖人口完成生平第一次报警。~~是女生眼中的好人~~
